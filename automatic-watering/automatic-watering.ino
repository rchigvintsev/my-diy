...
///  - режим настройки позволяет редактировать интервал между поливами (дни / часы / минуты) и длительность полива
///    (минуты / секунды);
///  - значения параметров сохраняются в EEPROM и защищены CRC-32;
///  - на LCD 16x2 выводится время до следующего полива, во время полива — анимация лейки;
...
///  - кнопки — BUTTON_SET_PIN, BUTTON_LEFT_PIN, BUTTON_RIGHT_PIN (INPUT_PULLUP, активные при замыкании на GND);
...
#ifdef DEBUG
ArduLog logger("AutomaticWatering");
#endif

/// Кадры анимации «Лейка».
///
/// Структура: [номер_кадра][индекс_символа_8x5][строка_5x1].
/// Каждый кадр состоит из 8 пользовательских символов LCD, расположенных в две строки по 4 символа.
...
// Флаги, указывающие на то, что удержание кнопки уже было обработано. Защищают от повторных срабатываний на одно и
// то же физическое удержание.
...
///
/// Вызывается каждые 10 мс независимо от блокировок в loop(), что обеспечивает корректное распознавание клика и
/// удержания.
void updateButtons() {
  setButton.update();
  leftButton.update();
...
/// Управляет подсветкой LCD: гасит её после таймаута бездействия и включает при нажатии любой кнопки.
...
///
/// После последнего шага мастер автоматически закрывается, а параметры сохраняются в EEPROM.
...
///
/// При выходе из режима настройки параметры сохраняются в EEPROM. Действие запрещено, если помпа в данный момент
/// работает.
void handleSetButtonHold() {
  ...
  if (!pumpRunning && !setButtonHeld && setButton.isHeld()) {
    setButtonHeld = true;
    lcd.clear();
    // Сбрасываем предыдущие взаимодействия с кнопками, чтобы вход/выход из режима настройки не подхватил случайные
    // клики или удержания.
    leftButton.reset();
    rightButton.reset();
...
///
/// Применяет минимально допустимые ограничения: интервал полива не может стать равным нулю (минимум — 1 минута),
/// длительность полива — минимум 1 секунда.
...
///
/// На экране трижды мигает обновлённое время до полива — обратная связь пользователю. Действие запрещено в режиме
/// настройки и при работающей помпе.
...
///
/// Применяет ограничения: часы 0..23, минуты и секунды 0..59. Дополнительно проверяется отсутствие переполнения
/// unsigned long.
...
///
/// Используется для ручного запуска полива. Действие запрещено в режиме настройки и при уже работающей помпе.
...
/// Основная логика помпы: включает по таймеру, выключает по таймеру, обновляет соответствующий экран.
///
...
///
/// В зависимости от шага мастера показывает интервал между поливами либо длительность полива. Поле текущего шага
/// мигает.
...
///
/// При \p blinkCount > 0 функция блокирует loop() на \p blinkCount × 500 мс, однако обслуживание кнопок (по прерыванию
/// Timer1) и подсветки LCD продолжается.
///
...
void displayTimeUntilNextWatering(unsigned long time, unsigned int blinkCount) {
  ...
  // Неблокирующее мигание: во время ожидания продолжаем обслуживать подсветку. Прерывание Timer1 продолжает обновлять
  // состояния кнопок независимо.
...
///
/// Во время ожидания продолжает обслуживаться подсветка LCD (updateLcdBacklight), а опрос кнопок продолжается из
/// прерывания Timer1 параллельно.
///
...
///
/// Кадры сменяются с интервалом WATERING_ANIMATION_FRAME_INTERVAL_MILLIS. Используются 8 пользовательских символов LCD
/// (createChar).
...
/// \param includedTimeUnits  Битовая маска включаемых единиц: бит 0 — дни, бит 1 — часы, бит 2 — минуты, бит 3 —
///                           секунды.
...
///
/// Применяется при действиях пользователя, чтобы свежее значение в редактируемом поле гарантированно было видно
/// (не оказалось в фазе «спрятано»).
...
///
/// Если контрольная сумма не совпадает, в EEPROM записываются значения по умолчанию и CRC пересчитывается. Это защищает
/// от случайного «мусора» при первом запуске или повреждении EEPROM.
void checkEeprom() {
...
///
/// Стандартный CRC-32: начальное значение 0xFFFFFFFF, табличный расчёт по полубайтам, финальная инверсия выполняется
/// один раз после прохода по всем байтам.
///
