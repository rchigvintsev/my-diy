/// \file ArduButton.h
/// \brief Обёртка над тактовой кнопкой с подавлением дребезга и распознаванием
///        клика и удержания.
///
/// Особенности:
///  - вход с подтяжкой к питанию (INPUT_PULLUP); кнопка считается нажатой
///    при логическом 0 на пине;
///  - программное подавление дребезга по таймауту;
///  - три состояния: «отпущена», «нажата», «удерживается»;
///  - метод isClicked() возвращает true ровно один раз после полного цикла
///    «нажатие — отпускание» (если не было перехода в HELD);
...
/// \brief Тактовая кнопка с подавлением дребезга и распознаванием
///        клика и удержания.
class ArduButton {
private:
	byte _pin;
...
public:
	...
	///
	/// Должен вызываться регулярно (минимум каждые несколько миллисекунд),
	/// например, из обработчика прерывания таймера.
	void update(void);

	/// \brief Сообщает, был ли зарегистрирован клик с момента предыдущего вызова.
	///
	/// Возвращает true ровно один раз после полного цикла «нажатие — отпускание»,
	/// не перешедшего в удержание. Внутренний счётчик кликов сбрасывается.
	boolean isClicked(void);

	/// \brief Сообщает, находится ли кнопка в состоянии «нажата»
	...
	///
	/// Полезно после смены контекста (например, входа или выхода из режима
	/// настройки), чтобы не подхватить случайные клики или удержания,
