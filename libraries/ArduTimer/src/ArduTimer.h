...
///  - корректно работает при переполнении millis() (раз в ~49.7 суток)
///    за счёт свойств беззнаковой арифметики;
///  - при последовательных пропусках срабатываний интервал «догоняет» текущее
///    время, сохраняя ритм без накопления ошибки.
///
...
public:
	...
	///
	/// Возвращает true, если с момента последнего срабатывания или сброса
	/// прошло не меньше интервала. При интервале 0 всегда возвращает true.
